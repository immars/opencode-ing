...
* 如果TASK.md 用户任务不为空，则让agent 检查一次用户任务。注入：工作目录的根目录； L9中的SOUL.md，PEOPLE.md，TASK.md；当前日期时间。
* 如果CRON.md 用户的定时任务不为空，并且匹配当前的触发时间，则让agent执行一次定时任务。注入：工作目录的根目录； L9中的SOUL.md，PEOPLE.md，CRON.md中匹配当前时间的任务条目；当前日期时间。
  * **session选择**：如果任务指定了 `author` 字段（飞书聊天ID），则使用对应的 `Chat {chatId}` session 执行任务；否则使用默认的 `Assistant Managed Session`。
...
* 注入context 的时机，从每次消息交互，变成每次新建session的时候。
